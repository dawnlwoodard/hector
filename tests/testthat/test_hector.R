...
test_that('Test fetchvars_all.', {
    ...
    lapply(c(vars_date, vars_nodate), function(v) {
        curr_cvars <- control_vars[control_vars$variable == v,]  # Control val
        curr_tvars <- test_vars[test_vars$variable == v,]        # Test var
        expect_equal(curr_cvars$value, curr_tvars$value, info = paste0(v, ' values must be equal'))
    })
})
